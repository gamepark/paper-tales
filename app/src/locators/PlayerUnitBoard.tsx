  ...
  getBaseCoordinates(location: Location, context: MaterialContext) {
    ...
    switch (position) {
      case Position.TopLeft:
        return players === 4 ? { x: -48, y: -30 } : { x: -40, y: -36 }   // TDB
      case Position.BottomCenter:
        return { x: -6, y: 15 }     // TDB 
      case Position.TopRight:
        return players === 3 ? { x: -50, y: -25 } : players === 4 ? { x: 48, y: -30 } : { x: 40, y: -36 }  // TDB
      case Position.BottomLeft:
        return players === 2 ? { x: -47, y:-36 } : players === 3 ? { x: -5, y: -4 } : players === 4 ? { x: -48, y: 15 } : { x: -60, y: 6 }    // TDB > 2
      case Position.BottomRight:
        return players === 2 ? { x: 47, y: -36 } : players === 3 ? { x: 50, y: -25 } : players === 4 ? { x: 48, y: 15 } : { x: 60, y: 6 }   // TD > 2
    }
  }
