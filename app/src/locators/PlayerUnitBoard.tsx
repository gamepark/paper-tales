...
class PlayerUnitBoard extends Locator {
  ...
  getCoordinates(location: Location, context: MaterialContext) {
    ...
    const signForMirror = (position === Position.BottomRight || position === Position.TopRight) ? -1 : 1
  ...
  getBaseCoordinates(location: Location, context: MaterialContext) {
    ...
    switch (position) {
      ...
      case Position.TopRight:
        return { x: 50, y: -36 }    // TDB
      case Position.BottomLeft:
        return players === 2 ? { x: -47, y:-36 } : players === 3 ? { x: -40, y: -36 } : { x: -48, y: -9 }    // TDB > 2
      case Position.BottomRight:
        return players === 2 ? { x: 47, y: -36 } : players === 3 ? { x: 50, y: 2 } : { x: 58, y: -9 }    // TD > 2
    }
  }
