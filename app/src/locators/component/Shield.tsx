...
export const ShieldIcon: FC<{ location: Location }> = ({ location }) => {
  ...
  const unit: MaterialItem | undefined = rules
    .material(MaterialType.Unit)
    .filter((_i, index) => index === location.parent)
    .getItem()
  const player = usePlayerId()
  const war = useMemo(() => new War(rules.game), [rules.game, player])
  const unitPower =
    unit === undefined || player === undefined || unit.id === undefined
      ? undefined
      : war.getUnitPower(player, unit)
