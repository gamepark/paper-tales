...
export class BuildWithSubstitution extends SimultaneousRule {
  ...
  getActivePlayerLegalMoves(playerId: number): MaterialMove[] {
    ...
    buildHelper.getPlayerBuildingUnplayed(playerId).filter(buildItem =>
      this.canBuildWithSubstitution(playerId, buildingCardCaracteristics[buildItem.id].cost2, 0)
    ).moveItems({
    ...
    if (playerGold >= fieldCost) {
      // Niveau 1
      buildHelper.getPlayerBuildingUnplayed(playerId).filter(buildItem =>
        this.canBuildWithSubstitution(playerId, buildingCardCaracteristics[buildItem.id].cost1, fieldCost) || this.canBuildWithSubstitution(playerId, buildingCardCaracteristics[buildItem.id].cost1Alternate, fieldCost)
      ).moveItems({
        type: LocationType.PlayerBuildingBoard,
        player: playerId,
        y: buildHelper.getPlayerBuildingQuantity(playerId),
      ...
      // Niveau 2
      buildHelper.getPlayerBuildingUnplayed(playerId).filter(buildItem =>
        this.canBuildWithSubstitution(playerId, [...buildingCardCaracteristics[buildItem.id].cost1, ...buildingCardCaracteristics[buildItem.id].cost2], fieldCost) || this.canBuildWithSubstitution(playerId, [...buildingCardCaracteristics[buildItem.id].cost1Alternate, ...buildingCardCaracteristics[buildItem.id].cost2], fieldCost) || this.canBuildWithSubstitution(playerId, [...buildingCardCaracteristics[buildItem.id].cost1, ...buildingCardCaracteristics[buildItem.id].cost2Alternate], fieldCost) || this.canBuildWithSubstitution(playerId, [...buildingCardCaracteristics[buildItem.id].cost1Alternate, ...buildingCardCaracteristics[buildItem.id].cost2Alternate], fieldCost)
      ).moveItems({
        type: LocationType.PlayerBuildingBoard,
        player: playerId,
        y: buildHelper.getPlayerBuildingQuantity(playerId),
  ...
  canBuildWithSubstitution(playerId: number, cost: Resources[], fieldCost: number): boolean {
    ...
    if ((missingResources.some(res => res === Resources.Wood) && !canReplaceWoodByGold)
