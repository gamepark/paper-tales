...
export class AgeHelper extends MaterialRulesPart {
  ...
  getAgeTokenOnUnit(unit: MaterialItem<number, number, any>): number {
    return this.material(MaterialType.Age).location(LocationType.PlayerUnitBoard).player(this.player).getItems(item => item.location.x === unit.location.x && item.location.y === unit.location.y).length
  }
  ...
  getAgeTokensOnDyingUnits(player: number): number {
    ...
    this.material(MaterialType.Unit).location(LocationType.PlayerUnitBoard).player(player).getItems().forEach(unit => {
      if (this.isUnitDying(unit)) {
        ageTokenToReturn += this.getAgeTokenOnUnit(unit)
      }
    })
    return ageTokenToReturn
  }

  isUnitDying(unit: MaterialItem<number, number, any>): boolean {
    const effects = this.getUnitAgeEffects(unit)
    const specialDyingEffect = effects.find(isSpecialDyingCondition)
    const unitsSavedByMysticEffect: number[] = this.remind(Memory.UnitSavedWithMystic, this.player)
    ...
    if (specialDyingEffect === undefined) {
      return this.getAgeTokenOnUnit(unit) >= 1
    }

    if (specialDyingEffect.dyingFromAmount === false) {
      return false
    } else {
      return this.getAgeTokenOnUnit(unit) >= specialDyingEffect.dyingFromAmount
    }
  }
  ...
  getUnitAgeEffects(unit: MaterialItem<number, number, any>): AgeEffect[] {
    ...
    this.material(MaterialType.Unit).location(LocationType.PlayerUnitBoard).player(this.player)
